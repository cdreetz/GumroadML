...
class Ranking(object):
	...
	def build_nework(self):
		...
		input_title = tf.keras.Input(shape=(None, ), name='product_name')
		inp_product_liked = tf.keras.layers.Input(shape=(None,), name='like')           #a product review of 3 or more stars is a like
		inp_product_disliked = tf.keras.layers.Input(shape=(None,), name='dislike')         #a product review of less than 3 stars is a dislike
		input_category = tf.keras.Input(shape=(None, ), name='category')            #a product category
		...
		labels_liked_embeddings = labels_embedding_layer(inp_product_liked)
		l2_norm_liked = l2_norm_1(labels_liked_embeddings)
		avg_liked = avg_embeddings(l2_norm_liked)

		labels_disliked_embeddings = labels_embedding_layer(inp_product_disliked)
		l2_norm_disliked = l2_norm_1(labels_disliked_embeddings)
		avg_disliked = avg_embeddings(l2_norm_disliked)

		labels_genre_embeddings = labels_embedding_layer(input_category)
