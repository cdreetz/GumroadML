...
class CandidateGeneration(object):
    ...
    def build_nework(self):
        input_purchase_hist = tf.keras.Input(shape=(None, ), name='purchase_hist')            #history of a users past product subscriptions and purchases
        input_purchase_hist_time = tf.keras.layers.Input(shape=(None,), name='purchase_hist_time')
        input_search_hist = tf.keras.layers.Input(shape=(None,), name='search_hist')            #history of a users past searches
        ...
        #--- features
        features_embeddings = features_embedding_layer(input_purchase_hist)         #first layer where user histories are embedded
        l2_norm_features = l2_norm_1(features_embeddings)
        avg_features = avg_embeddings(l2_norm_features)

        labels_watch_embeddings = labels_embedding_layer(input_purchase_hist_time)
        ...
        model = tf.keras.models.Model(
            inputs=[input_purchase_hist, 
                    input_purchase_hist_time, 
                    input_search_hist,
                    input_example_age,
                    ],
            outputs=[outputs]
